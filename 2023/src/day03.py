...
def checkAreaForSymbol(i, j, length) -> bool:
    ...
    if (
        (above and any(c and not c.isnumeric() for c in above))
        or (below and any(c and not c.isnumeric() for c in below))
        or (left and not left.isnumeric())
        or (right and not right.isnumeric())
    ):
        return True
    else:
        return False
